...
@pytest.mark.parametrize(
    ("input_str", "expected_output_str", "expect_error_log"),
    [
        # 角度のテスト
...
def test_cmd_calib_main_input(
    ...
    mocker.patch.object(
        calib_app._log,
        "error",
        side_effect=lambda msg, *args: print(msg % args, file=sys.stderr),
    )  # エラーログをstderrに出力するようにモック
...
def test_cmd_calib_main_commands(calib_app, mocker, capsys):
    ...
    commands = ["h", "c", "n", "x", "g", "sc", "sn", "sx", "invalid", "q"]
