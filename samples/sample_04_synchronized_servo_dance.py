...
def synchronized_servo_dance():
    ...
    # サーボ設定ファイルのパス
    config_file = (
        "samples/sample_servo.json"  # samplesディレクトリのファイルを使用
    )
    ...
    try:
        ...
        while (time.time() - start_time) < duration:
            ...
            # サーボ1の角度計算（位相0）
            angle1 = offset + amplitude * math.sin(
                2 * math.pi * frequency * elapsed_time
            )

            # サーボ2の角度計算（位相π/2 = 90度ずらし）
            angle2 = offset + amplitude * math.sin(
                2 * math.pi * frequency * elapsed_time + math.pi / 2
            )

            # サーボ3の角度計算（位相π = 180度ずらし）
            angle3 = offset + amplitude * math.sin(
                2 * math.pi * frequency * elapsed_time + math.pi
            )
